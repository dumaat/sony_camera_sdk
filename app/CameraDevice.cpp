...
{
    load_properties();
    if (m_prop.extended_shutter_speed.possible.size() < 1) {
...
{
    load_properties();
    if (m_prop.customwb_size_setting.possible.size() < 1) {
...
{
    load_properties();
    if (m_prop.time_shift_shooting.possible.size() < 1) {
...
void CameraDevice::execute_get_zoom_and_focus_preset()
{
    ...
    if (CR_SUCCEEDED(ret) && 0 < presetNums) {
        for (CrInt32u i = 0; i < presetNums; i++)
        {
            ...
            {
                wctomb(&dspValue[k], (wchar_t)ZoomAndFocusPreset[loadNum].lensModelName[1 + k]);
            }
...
void CameraDevice::camera_button_function(bool multiFlg)
{
    ...
    tout << "Camera Button Function Status: " << format_camera_button_function_status(m_prop.camera_button_function_status.current) << std::endl;
    tout << std::endl;
